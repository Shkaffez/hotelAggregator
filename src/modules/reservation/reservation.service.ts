...
export class ReservationService implements IReservation {
    ...
    async removeReservation(id: ID): Promise<void> {
        ...
        if (!reservation) {
            throw new HttpException('this reservation not exist', HttpStatus.BAD_REQUEST);
        }
    ...
    async deleteCurrentUserReservations(userId: ID, reservationId: ID): Promise<void> {
        if (!mongoose.Types.ObjectId.isValid(reservationId)) {
            throw new HttpException('not valid id', HttpStatus.BAD_REQUEST);
        }
        const reservation = await this.ReservationModel.findById({ _id: reservationId });
        if (!reservation) {
            throw new HttpException('this reservation not exist', HttpStatus.BAD_REQUEST);
        }
        if (!reservation.userId.equals(userId)) {
            throw new HttpException('this reserve belongs to another user', HttpStatus.FORBIDDEN);
        }
    ...
    async deleteUserReservations(userId: ID, reservationId: ID): Promise<void> {
        ...
        if (!reservation.userId.equals(userId)) {
            throw new HttpException('this reserve belongs to another user', HttpStatus.BAD_REQUEST);
        }
        await this.ReservationModel.findByIdAndDelete({ _id: reservationId });
        return;
    }
